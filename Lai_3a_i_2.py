...
def get_data():
    ...
    if r.status_code == 200:
        ...
        df.to_csv('Lai_stock_holder_data_horizontal.csv', index=False)
        # Writing stock data to an Excel file 
        df.to_excel('Lai_stock_holder_data_horizontal.xlsx', index=False)

        path = os.getcwd()
        print("Download links:")
        print(f"JSON: {path}\\Lai_stock_holder_data.json")
        print(f"CSV: {path}\\Lai_stock_holder_data_horizontal.csv")
        print(f"XLSX: {path}\\Lai_stock_holder_data_horizontal.xlsx")
